...
namespace Utils {
	...
	bool String::__dim(const uint &sz)
	{
		...
		if ( !Utils::memset( ccast<byte*>( newbuf ), 0, newsz ) ) {
			delete[newsz] newbuf;
			return false;
		}

		Utils::memcpy( ccast<byte*>( newbuf ), ccast<byte*>( _buffer ), _bufsz );
		__clean( );
		_buffer = newbuf;
		_bufsz = newsz;
		return true;
	...
	bool String::__copy_ct(const char *str, const uint &sz)
	{
		if ( !__dim( sz ) )
			return false;
		return Utils::memcpy( ccast<byte*>( _buffer ), ccast<byte*>( str ), sz );
	}
	...
	bool String::__shrink(const uint& newsz)
	{
		...
		Utils::memcpy( ccast<byte*>( newbuf ), ccast<byte*>( _buffer ), newsz );
		__clean( );
		_buffer = newbuf;
		_bufsz = onewsz;
		return true;
		...
		: _buffer(nullptr), _bufsz(0)
	{
		_buffer = new char[20];
		_bufsz = 20;
		Utils::memset( ccast<byte*>( _buffer ), 0, _bufsz );
	}
	...
	String String::substr(const std::pair<uint, uint> &pos) const 
	{
		...
		if ( !Utils::memcpy( ccast<byte*>( buffer ), ccast<byte*>( &_buffer[pos.first] ), pos.second ) )
			return "";
		buffer[pos.second] = '\0';
		return buffer;
	}
	...
	bool Utils::String::contains(const String &str) const
	{
		return find( str ) != bad;
	}

	bool Utils::String::hasCapital() const
	{
		for ( auto&x : *this )
			if ( x > 'A' && x < 'Z' )
				return true;
	...
	bool Utils::String::hasNumerical() const
	{
		for ( auto&x : *this )
			if ( x > '0' && x < '9' )
				return true;
	...
	void Utils::String::erase(const uint & pos, const uint & off)
	{
		return erase( { pos, off } );
	}

	void Utils::String::erase(const std::pair<uint, uint>& pos)
	{
		auto str = __concat( substr( 0, pos.first ), substr( pos.first + pos.second ) );
		__copy_ct( str._buffer, str.length( ) + 1 );
	}

	void Utils::String::insert(const std::pair<uint, uint>& pos, const String & str)
	{
		auto len = str.length( );
		__dim( length( ) + len );
	...
	void Utils::String::insert(const uint & pos, const uint & off, const String & str)
	{
		return insert( { pos, off }, str );
	}

	void Utils::String::insert(const uint &pos, const String &str)
	{
	...
	std::vector<Utils::String> Utils::String::split(const String& splitter) const
	{
		std::vector<Utils::String> ret;
		uint pos = 0;
		auto copy = *this;
	...
	String Utils::String::lower() const
	{
		String ret = *this;
		for ( auto& x : ret )
			x = ::tolower( x );
	...
	String Utils::String::operator+(const String &str) const
	{
		String string = *this;
